...
class Checkoutcontroller extends Controller
{
        ...
        if($request->payment_mode=='COD')
        {
          ...
          $uniquemcid= "MTUID".rand(000000,999999);
            $data=   array (
                ...
                'amount' =>  $amount*100,
                'redirectUrl' => url('https://albfoodproducts.in/phonepe-response'),
                'redirectMode' => 'POST',
                'callbackUrl' => url('https://albfoodproducts.in/phonepe-response'),
    ...
    if($input['code']=='PAYMENT_ERROR')
    {
        ...
        ->update(['payment_id'=>$input['providerReferenceId'],'payment_state'=>$input['code'],'status'=>'Order-Not-Placed','payment_status'=>$getdatajson->data->responseCode]);
        
           return redirect('my-order')->with('status',"Order Not Placed Successfully");
    }else{
          $order = DB::table('orders')
        ->where('uniqueMerchantID',$input['transactionId'])
        ->update(['payment_id'=>$input['providerReferenceId'],'payment_state'=>$input['code'],'status'=>'Pending','payment_status'=>$getdatajson->data->responseCode]);
