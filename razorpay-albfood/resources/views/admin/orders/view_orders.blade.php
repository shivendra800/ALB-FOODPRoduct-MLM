                            ...
                            <div class="row">
                                    ...
                                    <table  id="myTable" class="table table-bordered">
                                        ...
                                        <tbody>
                                            ...
                                            <tr>
                                                ...
                                                <td>{{$item->price}}</td>
                                                <td>
                                                    <img src="{{asset('admin_asset/uploads/product/'.$item->product->image)}}" alt="" width="50px;">
                                                </td>
                                ...
                                @endphp
                                   <h4 class="px-2"> Total : <span class="float-end">{{$orders->total_price}}</span></h4>
                                     ...
                                     <h4 class="px-2">CGST 6% : <span class="float-end">{{$cgst_sgst}}</span></h4>
                                      <h4 class="px-2">SGST 6% : <span class="float-end">{{$cgst_sgst}}</span></h4>
                                       <h4 class="px-2">Total GST 12% : <span class="float-end">{{$orders->gstamt}}</span></h4>

                                    <!--<h4 class="px-2">Total GST Amount : <span class="float-end">{{$orders->gstamt}}</span></h4>-->
                                    <h4 class="px-2">Grand Total : <span class="float-end">{{$orders->totalwithgst}}</span></h4>
                                    <div class="mt-5 px-2">
                                                ...
                                                <div class="col-md-6">
                                                  ...
                                                  <select class="form-select" name="order_status">
                                                   ...
                                                   <select class="form-select" name="order_status">
                                                    ...
                                                    <select class="form-select" name="order_status">
                                                     {{-- <option {{$orders->status == 'Out For Delivery' ? 'selected' : ''}} value="Out For Delivery">Out For Delivery</option> --}}
                                                     <option {{$orders->status == 'Delivered' ? 'selected' : ''}} value="Delivered">Delivered</option>
                                                     <option {{$orders->status == 'Cancel' ? 'selected' : ''}} value="Cancel">Cancel</option>
                                                     <option {{$orders->status == 'Undeliverable' ? 'selected' : ''}} value="Undeliverable">Undeliverable</option>
                                                    @endif
                                                    </select>
                                                </div>
                                                <div class="col-md-6">
                                                    ...
                                                    <label for="">Order Status</label>
                                                     <select name="payment_status" class="form-select" required>
                                                       <option {{$orders->payment_status == 'Paid' ? 'selected' : ''}} value="Paid">Paid</option>
                                                       <option {{$orders->payment_status == 'Unpaid' ? 'selected' : ''}} value="Unpaid">Unpaid</option>
                ...
                success: function(data) {
                    ...
                    for (var i = 0; i < data.data.length; i++) {
                        if (deliveryBoy == data.data[i].id) {
                            option += "<option selected value=" + data.data[i]['deliveryboy'].id + ">" + data.data[i]['deliveryboy'].name + "</option>";
                        } else {
                            option += "<option value=" + data.data[i]['deliveryboy'].id + ">" + data.data[i]['deliveryboy']
                                .name + "</option>";
                        }
                    }
